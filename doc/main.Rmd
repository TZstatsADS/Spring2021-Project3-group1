...
So we choose the parameter, ntrees = 100, mtry =50. 
```{r find para}
source("../lib/rand_f.R")
# training rf
...
```{r load para}
load("../output/res_pa_RF.RData")
plot(ntrees, res_pa_rf[,1], type = "l",ylim=c(0.8,0.85))
points(ntrees,res_pa_rf[,2]+0.3)
```

###### 5.1.3 Train the model with tunning parameter
...
if (run.train.RF) {
  tm_rfTrain <- system.time(
     randomforest_fit <- train_RF(feature_train, label_train, ntree = 100, mtry = 50)
)
...
###### 5.1.4 Test the model by train data
```{r testtrain}
# testing rf
tm_rf_train_test = NA
...
###### 5.1.5 Test the model by test data
...
###### 5.1.6 Summary of the random forest model
```{r summary rf}
